...
function renderBoard(board) { // From Pacman
    var strHTML = ''
    // console.table(board);
...
function getMoveDiff() { // return random movement direction
...
function getNextLocation(eventKeyboard) { // next Pacman location
...
function getRandomIntInc(min, max) { // Maximum is inclusive
    min = Math.ceil(min);
    max = Math.floor(max);
...
function handleKey(event) { // Move the player by keyboard arrows
...
function startStop(elBtnStart) { /* Toggle StartStop */
...
<body onload="init()">
    <div class="sky">
        <div class="header">
            ...
            <button onclick="startStop(this)" class="elBtnStart">start </button>
        </div>
        <div class='timer'><span id="hour">00</span> :
            <span id="min">00</span> :
            <span id="sec">00</span> :
            <span id="milisec">00</span> </div>
